...
function getSocialIcon(title: string) {
  ...
  if (t === "linkedin")
    return (
      <svg viewBox="0 0 448 512" fill="currentColor" className="size-3">
    ...
    return (
      <>
        ...
        <Image
          ...
          width={14}
          height={14}
          className="size-3 hidden dark:block"
        />
      </>
    );
  if (t === "email") return <MailIcon className="size-3" />;
...
export function Hero() {
  ...
  return (
    <Panel className="flex flex-col" hideTopLine>
      ...
      <div className="relative h-[100px] sm:h-[250px] w-full p-2">
        <div className="relative size-full overflow-hidden bg-background/50">
          <div className="overflow-hidden p-2 sm:p-3 h-full">
            <div className="h-full min-h-[60px] w-full bg-[radial-gradient(var(--pattern-foreground)_1px,transparent_0)] bg-size-[10px_10px] bg-center px-[5px] sm:min-h-[110px] [--pattern-foreground:color-mix(in_oklab,var(--color-zinc-400)_60%,transparent)] dark:[--pattern-foreground:color-mix(in_oklab,var(--color-zinc-600)_60%,transparent)]" />
          </div>
        </div>
      </div>

      {/* Header section wrapped in Panel */}
      <Panel className="flex w-full flex-col rounded-none border-x-0 border-t-0 bg-transparent gap-4 p-6 shadow-none sm:flex-row sm:gap-2">
        <div className="relative flex items-center md:size-32 sm:size-24 size-24">
          <Image
            src={PROFILE_IMAGES.light}
            alt={`${USER.displayName}'s avatar`}
            className="rounded-xl border border-solid p-0.5 relative dark:hidden"
            fill
            unoptimized
          />
          <Image
            ...
            alt={`${USER.displayName}'s avatar`}
            className="rounded-xl border border-solid p-0.5 relative hidden dark:block"
            fill
            unoptimized
          />
        </div>
        ...
        <div className="flex flex-1 flex-col justify-end pl-0 sm:pl-2">
          ...
          <Container delay={0.1}>
            <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
              ...
              <div className="flex items-center gap-2 pt-0.5 sm:pt-0 sm:pb-0.5">
                {heroSocials.map((link) => (
                  <SimpleTooltip key={link.title} content={link.title}>
                    <a
                      href={link.href}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex size-9 items-center justify-center rounded-full sm:rounded-[10px] border border-border bg-background/70 transition-colors hover:border-foreground/65 hover:bg-zinc-100/60 dark:hover:bg-zinc-800/60"
      ...
      <Container delay={0.2}>
        <div
          className="px-6 pt-6 pb-2 text-sm leading-5 font-normal text-muted-foreground [&>p]:mb-4 [&_strong]:font-semibold [&_strong]:text-foreground"
          style={{
            fontFamily:
